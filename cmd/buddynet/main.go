...
func registerFlags(fs *flag.FlagSet) *cliFlags {
	return &cliFlags{
		...
		allowCIDR:         fs.String("allow-cidr", "", "relay/handshake: comma-separated CIDRs allowed to reach the server role(s); a disallowed source is refused before it can occupy a connection slot -- on the relay before any crypto, on the handshake server after the TLS handshake QUIC already ran (SECURITY.md 5.5). On the HANDSHAKE server empty means open to all; on the RELAY it is one of the two authorization policies (the other is --server-key) and the relay refuses to start with neither — 0.0.0.0/0 and ::/0 are refused, an open relay is not a supported configuration"),
		...
		peerRelay:         fs.String("peer-relay", "", "buddy: relay `HOST:PORT` to fall back to when the direct path fails, in BOTH modes. With a handshake server: the relay endpoint comes from local trust only — this flag, or else the server's offer if (and only if) it sits on the --server HOST (the host is what is trusted, the port is the server's to pick); an offer on any other host is skipped and logged. In direct mode there is no server to mint a ticket, so the relay must accept ticket-free binds (scope it with --allow-cidr)"),
		...
		peersFile:         fs.String("peers-file", "", "buddy: MultiPeer manifest `PATH` (YAML: buddies with key/token/name/expose per entry; legacy line format still read — run 'peers migrate'); maintains a tunnel to every listed buddy at once (Model A, each pinned). Use --vip-listen to route to them. Mutually exclusive with --invite/--join/--lazy"),
