...
// challenge is a fixed, compact binary message (prefix + raw cookie bytes). It is
// smaller than any REALISTIC bind, but not smaller than the smallest one the
// parser accepts (a one-character session token is 17 bytes against a 23-byte
// challenge), so the sender compares the two lengths and stays silent rather than
// amplifying — the same gate the handshake server's cookie has. It closes the relay's
...
// MinSessionTokenLen is the shortest session token a relay will act on. The token
// is the ONLY thing that splices two legs together, so a short one is guessable —
// a third party that hits it lands in someone else's session (the leg cap then
// costs the real partner its slot). Buddies derive it in role.sessionToken as 16
// bytes of SHA-256, i.e. 22 base64url characters, so this floor is well below what
// any real client sends and rejects nothing legitimate.
//
// It also keeps the challenge gate in Server.bind out of reach of honest traffic:
// the smallest bind this admits is 32 bytes against a 23-byte challenge.
