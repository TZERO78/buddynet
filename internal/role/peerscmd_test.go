...
func TestPeersAddDedupAndParse(t *testing.T) {
	...
	dir := t.TempDir()
	path := filepath.Join(dir, "sub", "peers") // dir created by add
	known := filepath.Join(dir, "known_peers")

	if err := PeersAdd(path, known, aB64, "boot-a", "", ""); err != nil {
		t.Fatalf("add: %v", err)
	}
	...
	if err := PeersAdd(path, known, "not-a-key", "", "", ""); err == nil {
...
func TestPeersRemoveRevokesBoth(t *testing.T) {
	...
	for _, k := range []string{aB64, bB64} {
		if err := PeersAdd(peers, known, k, "tok", "", ""); err != nil {
...
func TestPeersListSmoke(t *testing.T) {
	...
	peers := filepath.Join(dir, "peers")
	known := filepath.Join(dir, "known_peers")
	if err := PeersAdd(peers, known, bcrypto.PubKeyB64(a), "boot-a", "", ""); err != nil {
		t.Fatal(err)
	}
...
func TestPeersRemoveByShortKey(t *testing.T) {
	...
	peers := filepath.Join(dir, "peers")
	known := filepath.Join(dir, "known_peers")
	if err := PeersAdd(peers, known, aB64, "tok", "", ""); err != nil {
		t.Fatalf("add: %v", err)
	}
